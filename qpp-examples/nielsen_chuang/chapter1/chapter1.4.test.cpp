...
TEST(chapter1_4, hadamard_transform)
{
    for(auto&& n : { 3, 4, 5 })
    {
        auto const _2_pow_n = qube::maths::pow(2u, n);
...
namespace
{
    auto random_function(int n)
    {
        ...
        auto f = std::vector<qpp::idx>(qube::maths::pow(2, n));
        for (auto& v : f)
            v = d(gen);
        return f;
    }
...
TEST(chapter1_4, deutsch_algorithm)
{
    ...
    for(auto&& f : functions())
    {
        ...
        if(f[0] == f[1])
            EXPECT_MATRIX_CLOSE(psi2, qube::maths::pow(-1, f[0]) * 0.5 * qpp::kron(0_ket + 1_ket, 0_ket - 1_ket), 1e-12);
        else
            EXPECT_MATRIX_CLOSE(psi2, qube::maths::pow(-1, f[0]) * 0.5 * qpp::kron(0_ket - 1_ket, 0_ket - 1_ket), 1e-12);

        auto const psi3 = qpp::apply(psi2, qpp::gt.H, { 0 });
        if(f[0] == f[1])
            EXPECT_MATRIX_CLOSE(psi3, qube::maths::pow(-1, f[0]) * inv_sqrt2 * qpp::kron(0_ket, 0_ket - 1_ket), 1e-12);
        else
            EXPECT_MATRIX_CLOSE(psi3, qube::maths::pow(-1, f[0]) * inv_sqrt2 * qpp::kron(1_ket, 0_ket - 1_ket), 1e-12);

        EXPECT_MATRIX_CLOSE(psi3, qube::maths::pow(-1, f[0]) * inv_sqrt2 * qpp::kron(qpp::mket({ (f[0] + f[1]) % 2 }), 0_ket - 1_ket), 1e-12);
...
namespace
{
    auto random_constant_function(int n)
    {
        ...
        return std::vector<qpp::idx>(qube::maths::pow(2, n), f[0]);
    }
    auto random_balanced_function(int n)
    {
        auto f = std::vector<qpp::idx>(qube::maths::pow(2, n), 0u);
...
TEST(chapter1_4, bitwise_inner_product)
{
    ...
    auto constexpr _2_pow_n = qube::maths::pow(2ul, n);
...
TEST(chapter1_4, deutsch_jozsa_algorithm)
{
    ...
    for(auto&& i : { 0, 1 })
    {
        ...
        auto const expected_psi2 = qpp::kron(std::transform_reduce(policy, range.begin(), range.end()
            ...
            , [&](auto&& i)
        {
            return (qube::maths::pow(-1, f[i]) * Eigen::VectorXcd::Unit(_2_pow_n, i)).eval();
        }), inv_sqrt_2_pow_n * inv_sqrt2 * (0_ket - 1_ket));
            ...
            , [&](auto&& x)
        {
            return std::transform_reduce(std::execution::seq, range.begin(), range.end()
                ...
                , [&](auto&& z)
            {
                return (qube::maths::pow(-1, (std::popcount(x & z) + f[x]) % 2) * Eigen::VectorXcd::Unit(_2_pow_n, z)).eval();
            });
